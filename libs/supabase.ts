import { createClient } from "@supabase/supabase-js";
import AsyncStorage from "@react-native-async-storage/async-storage";
// Create a single supabase client for interacting with your database 
const supabase = createClient(
  "https://fvpehbakvabuhpymovni.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ2cGVoYmFrdmFidWhweW1vdm5pIiwicm9sZSI6ImFub24iLCJpYXQiOjE2NDc3OTc3OTMsImV4cCI6MTk2MzM3Mzc5M30.jhx7DQTBv6dTXmC-41Q6fi5CcBMxusayeuyCrQ3Aaes",
  {
    localStorage: AsyncStorage as any
  });

async function getCountTable(tableName: string, columnName: string) {
  let { data, count, ...others } = await supabase.from(tableName).select(columnName, {
    count: "exact",
    head: true
  });
  console.log("count", { data, count, others });
  return count;
}

export async function seedDatabase() {

  // if no data in "Categories" table, seed it
  if (await getCountTable("Categoria", "id") === 0) {
    console.log("Categoria", await supabase.from("Categoria").insert([
      {
        name: "cookies",
        image: "https://images.squarespace-cdn.com/content/v1/5b049ae0e2ccd1d3e8f8cbca/1622426699929-4E46O214GUI03H0W5Q8G/emandskyecakeco1.jpg"
      },
      {
        name: "pasteles",
        image: "https://i.pinimg.com/550x/7d/64/ec/7d64ecbbbc40762347c4490668d5d578.jpg"
      },
      {
        name: "shots",
        image: "https://scontent.fhex4-2.fna.fbcdn.net/v/t1.6435-9/66376218_2893620974013968_8852730889084338176_n.jpg?_nc_cat=103&ccb=1-5&_nc_sid=9267fe&_nc_eui2=AeHSaitBpV9F2TZJeA1OvyarIyHHefP7baIjIcd58_ttomV48rtlpue0AM5hW0xMNoz5gFojuiztTdpMxicX4oVu&_nc_ohc=yU5bFzPcltYAX8sh_yZ&_nc_ht=scontent.fhex4-2.fna&oh=00_AT-NjO6P_gQC2ryrnghjTG67jn1LcqDbmWmeOCG_KowRnQ&oe=622EF07B"
      },
      {
        name: "postres",
        image: "https://i.ytimg.com/vi/2_FUW8y2J1M/maxresdefault.jpg"
      }
    ]));
  }

  // if no data in "Articulo" table, seed it
  if (await getCountTable("Articulo", "id") === 0) {

    console.log("Articulo", await supabase
      .from("Articulo")
      .insert([
        {
          image: "https://hips.hearstapps.com/hmg-prod.s3.amazonaws.com/images/raspberry-pink-velvet-cake-1574437052.jpg?crop=1xw:0.8333333333333334xh;center,top&resize=480:*",
          price: 500,
          description: "Pastel Dominicano de Frambuesa",
          name: "Pastel Dominicano de Frambuesa",
          id_categoria: 1
        },
        {
          image: "https://annaspasteleria.com/images/blocks/_imageBlock/DSC_9204web.jpg",
          price: 80,
          description: "Galletas LOVE",
          name: "Galletas LOVE",
          id_categoria: 1
        },
        {
          image: "data:image/jpeg;base64,",
          price: 100,
          description: "Shots",
          name: "Shots",
          id_categoria: 1
        },
        {
          image: "https://images-gmi-pmc.edge-generalmills.com/6cf26612-e351-4ad1-b55d-4ed2aace25e0.jpg",
          price: 10,
          description: "Shots",
          name: "Shots",
          id_categoria: 1
        },
        {
          image: "https://images-gmi-pmc.edge-generalmills.com/6cf26612-e351-4ad1-b55d-4ed2aace25e0.jpg",
          price: 80,
          description: "Brownie a la moda",
          name: "Brownie a la moda",
          id_categoria: 1
        },
        {
          image: "https://hips.hearstapps.com/hmg-prod.s3.amazonaws.com/images/raspberry-pink-velvet-cake-1574437052.jpg?crop=1xw:0.8333333333333334xh;center,top&resize=480:*",
          price: 20,
          description: "hola mundo",
          name: "hola mundo",
          id_categoria: 1
        },
        {
          image: "https://hips.hearstapps.com/hmg-prod.s3.amazonaws.com/images/raspberry-pink-velvet-cake-1574437052.jpg?crop=1xw:0.8333333333333334xh;center,top&resize=480:*",
          price: 20,
          description: "hola mundo",
          name: "hola mundo",
          id_categoria: 1
        }
      ]));
  }
}

export default supabase;

export function parseSupabase(result:any) {
  if (result.error)
    throw result.error;

  return result.data;
}

// if file is executed directly, not required to be imported
if (require.main === module) {
  seedDatabase();
}